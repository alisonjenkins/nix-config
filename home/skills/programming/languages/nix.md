...
## Repo shape
Flake layout varies between repos; check what's there before assuming a
convention. Common patterns:
...
- **Explicit imports**: a central `default.nix`/`flake.nix` lists every
  module by path. Adding a file *does* require updating the list.
- Modules commonly follow the options pattern: `options.<name>` plus
  `config = lib.mkIf cfg.enable { ... }`; confirm the repo uses it before
  assuming.
...
- **DRY is enforced.** A value used twice (SSH keys, hostnames, ports, mandate
  ...
  Before writing any literal (SSH key, IP, DNS name, hostname, git URL,
  password hash) grep the flake for it. If it exists, extract the existing
  occurrence into a shared file or a `flake.lib.<topic>` output and consume it
  from both places. Do not add the second copy.
- **Machine-specific settings go in the host's own configuration**, never in a
  ...
  applies to before putting it somewhere shared.
...
- **A lambda default on a module argument does not make it optional.**
  `{ someArg ? "fallback", ... }:` looks safe to omit, and is not: the module
  system resolves named arguments through `_module.args` and never consults
  the default, failing with `error: attribute 'someArg' missing`. Every
  configuration importing the module must pass the value, so a genuinely
  optional argument should be read as `args.someArg or "fallback"` from an
  `@ args` pattern instead. This bites when a module is shared between
  configurations with different argument sets, such as a NixOS host and a
