...
- `golangci-lint run` if the project has it configured; it aggregates
  `staticcheck`, `errcheck`, `gosec`, and others behind one command, so
  prefer it over remembering which individual linters the project uses.
...
- `errcheck` (via `golangci-lint`, or standalone) enabled and clean. Go's
  compiler does not force you to check an error return; nothing stops `f()`
  silently discarding the `error` it returned. This is the language's
  equivalent of Rust's `unwrap_used` deny: a lint turning a silently
  ignorable failure into a build-time one. Never `_ = f()` to silence it;
...
- **Wrap errors with `fmt.Errorf("doing X: %w", err)` at every boundary a
  ...
  `errors.Is`/`errors.As` for every caller above. Every wrap names the
  specific operation that failed (`"reading config %s: %w", path, err`), so a
  log line traces back to one call site, as `rust.md`'s per-site error
  variants do. Go's mechanism is a wrapped, named string instead of an enum
  variant; the goal is identical: never a bare `err` re-returned with no
  added context.
- **Define a typed/sentinel error for any failure a caller needs to branch
  on**, not a string to substring-match: `var ErrNotFound =
...
- Return `(T, error)`, not a zero value silently standing in for failure.
  A `nil, nil` return from a function whose contract says one implies the
  other is a common defect; keep the two orthogonal.
- Accept interfaces, return concrete structs: a parameter asks for the
  minimal interface it needs (`io.Reader`, not `*os.File`); a return type is
  the concrete type callers can act on without a type assertion.
...
- `context.Context` is the first parameter of any function that can block,
  call another service, or needs a deadline/cancellation. Never stored in a
  struct field, never `context.TODO()` outside a genuine migration stopgap.
...
- Every goroutine has an owner that can observe when it's done and
  ...
  tied to it is a leak waiting to happen, and any error it returns internally
  is silently dropped (a panic is never silent; an unrecovered one crashes
  the whole process). Go's form of `rust.md`'s "every spawn has an owner".
...
- Protect shared state with a `sync.Mutex`/`sync.RWMutex` held for the
  shortest scope that needs it, or pass ownership through a channel instead
  of sharing memory at all ("share memory by communicating"). See
  `concurrency.md` for the general discipline.
...
- **Loop variable capture**: pre-Go-1.22, a closure or goroutine launched
  inside `for _, v := range items` captured the same `v` across every
