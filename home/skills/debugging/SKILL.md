...
2. **Probe the layer closest to the fault.** Every layer above the broken one
   ...
   metrics are all one step removed from what is actually wrong.
...
Ask: *what would this look like if the thing I trust were lying?* Then check
that thing directly. Most long debugging sessions are spent re-verifying layers
that were never broken, because the first green signal was believed.
