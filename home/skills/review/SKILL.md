...
3. **Scope** — does the diff do more than the change requires? Unrelated
   refactors, silently widened behaviour, dead code left behind.
4. **Reuse** — does this duplicate something the codebase already has?
5. **Simplification** — is there a materially simpler shape with the same
   behaviour? Not stylistic preference; structural reduction.
6. **Efficiency** — only where it matters: work inside a loop that belongs
   outside it, an N+1 query, an unbounded buffer.
7. **Test coverage** — is the new behaviour actually pinned by a test that
