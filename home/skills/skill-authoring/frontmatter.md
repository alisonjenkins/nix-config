...
behaviour depends on it. Never put a *behavioural* extension — `paths`,
`context`, `disable-model-invocation`, `user-invocable` — on a shared skill;
those change what the skill does, so a runtime that ignores them runs it
differently.
