# Tide prompt configuration — Alisons-MacBook-Pro (performance-optimised)
# Based on tide_config.fish but with right prompt trimmed to tools actually used
# on this machine. Reduces per-render item-detection loops from 20 → 11,
# which helps under corporate AV scanning.
#
# Removed from right prompt: bun node python rustc java php pulumi ruby go
#                             distrobox toolbox crystal elixir zig
...
set -g tide_right_prompt_items status cmd_duration context jobs direnv gcloud kubectl terraform aws nix_shell time
