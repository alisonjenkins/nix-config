...
let
  ...
  awsKarpenterNodeConfig = { modulesPath, lib, pkgs, ... }: {
    ...
    # Pre-pull container images at nix build time using dockerTools.pullImage.
    # k3s auto-imports tar files from /var/lib/rancher/k3s/agent/images/ on start.
    # Hashes are generated by .github/scripts/update-prepull-hashes.sh and stored
    # in flake-modules/karpenter-prepull-images.json.
    # Images are placed directly into the repart image (not symlinked from the
    # nix store) so they don't bloat the store closure on the AMI.
