...
Args required; no default host. Read `.claude/skills/build/SKILL.md` first for the host
registry, groups, build procedure, and debug loop.
...
- `servers` hosts (download-server-1, home-kvm-hypervisor-1, home-storage-server-1,
  home-k8s-master-1) → **boot**, because many server changes don't apply cleanly live;
  config takes effect on next reboot.
...
"deploy node: yes" hosts). That file is ground truth, so grep it if unsure. Reject anything
...
because deploying while another host's fix is in flight risks shipping broken shared
modules.
...
- switch mode: deploy-rs auto-rollback + magic-rollback are on by default, so a deploy
