...
- `lionfishapi 2.7-fix-fix` drops `IAnimatedEntity`, so `arkana-mods-extras.nix` pins 2.6. Comment in extras explains; don't bump.
...
| `Trying to access unbound value: ResourceKey[...]` | registry-init | `DeferredHolder` resolved before its registry runs; usually a mod's mixin firing during another mod's RegisterEvent. Hard to fix without bumping the mixin source, so usually `disabled` |
