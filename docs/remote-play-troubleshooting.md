...
| Black stream, no `GameOverlay_MovieStream` line at all, game renders on the host and Shift+Tab draws Steam's overlay over it | Steam's game capture never started for this game (FH6 on DW-Proton). Compare the game's Proton with one that streams: `grep -A3 '"<appid>"' ~/.steam/steam/config/config.vdf`. Try the Proton of a game that works | PENDING.md item 15 |
