...
| `Desktop Black Frame` and never `Desktop PipeWire`, no `no more input formats` | Steam's desktop capture waits for portal consent. Look for an `xdg-desktop-portal-gnome` "Share Screen" window (`niri msg windows`), often behind the game; choose the `steam` output, tick "Remember this selection", Share | PENDING.md item 9 |
| Black stream after `Switching video stream ... to GameOverlay_MovieStream_<pid>`, no `Capture method set to Game` after it | Steam's game capture stalled. After six client reports (about 30s) with no capture, stream-mode moves focus to an empty workspace on the streamed output (or a non-Steam window if there is none) and back to the game at the next report (`game capture has not started` in its journal). By hand: `niri msg action focus-workspace <empty idx>`, wait a couple of seconds, then `niri msg action focus-window --id <game>`. Straight back is too fast, and a Steam window as the other one gets recorded instead: either way Steam stays on a desktop capture that shows black | |
| After a reconnect the camera works but movement and buttons don't | Which `/dev/input/event*` the game's `winedevice.exe` holds vs the newest `Microsoft X-Box 360 pad N`; `touch` the new node. stream-mode does this when `virtualgamepadinfo.txt` changes (`re-announced Steam gamepads` in its journal) | [0014](adr/0014-reannounce-steam-virtual-gamepads.md) |
| One button misbehaves (A won't select), the rest work | A mouse button held on extest's device after a Desktop-layout swap. Check with an `EVIOCGKEY` read of the `extest fake device` node; release by focusing an empty workspace (Desktop layout), pressing R2 once, and focusing the game again | PENDING.md item 13 |
| Picture frozen but the stream is alive (steady bitrate) and the game renders on the host | Steam's game capture froze after a rebind. Focus an empty workspace, wait a few seconds, focus the game again. Keep hands off R2 and the trackpad meanwhile | PENDING.md item 14 |
| Black stream, no `GameOverlay_MovieStream` line at all, game renders on the host and Shift+Tab draws Steam's overlay over it | The game presents into a child window, which Steam's game capture does not take (FH6, DX12). `xwininfo -root -tree`: a full-size unnamed child under the game's window means this. Stream it in desktop mode instead: make sure `Desktop PipeWire` capture works (row above), and give the game a resolution rule if it draws a corner of the output | PENDING.md item 15 |
| Game not fullscreen on the client, window left at its splash size | `niri msg -j windows`: `is_floating: true` on the game. niri floats a game whose first window is a fixed-size splash; stream-mode now tiles and fullscreens it. By hand: `niri msg action move-window-to-tiling --id <id>` then `set-window-fullscreen` | |
| A login box or launcher prompt never reaches the client | Steam streams the focused window. stream-mode takes focus back to the game only when focus falls to an older window; a window opened after the game keeps it. If it is hidden anyway, `niri msg action focus-window --id <dialog>` | |
| Stream goes black and the game mutes after Alt+Enter or a full screen change, or FH6 shrinks to 640x400 | FH6 opens an untitled, black, output-sized window with its own app id, and niri focuses it. FH6 then seems to leave full screen (inferred: its windowed size is 640x400). stream-mode takes focus back from an untitled window of the game when the game's window has a title (`lost focus; taking it back`). By hand: `niri msg action focus-window --id <titled window>`, then Alt+Enter in the game to return to full screen | PENDING.md item 15 |
| Voices or effects missing from a streamed game, music fine | `pactl list sink-inputs`: the game playing surround straight into `steam-streaming-playback` (channel map `aux0,aux1`) loses everything but the front pair. `journalctl --user -u steam-stream-mode \| grep audio` should show `audio for <client> through remote-play-…`; if it says `no … audio sink`, `/etc/steam-remote-play` is missing | [0015](adr/0015-positioned-sink-for-stream-audio.md) |
| Choppy/stuttering audio to a VR headset (video smooth), Beat Saber and other VR games | The originally-cited `driver_vrlink.txt` `CAudioJitterBuffer` signature could not be reproduced from surviving logs — don't trust that citation. Instead run `pw-top -b` during the session for host-side xruns, and check `streaming_log.txt`'s per-minute `Audio source [System Pulse]: mixed=, adjustment=, drop_before=, drop_after=` counters: zero host xruns and `drop_before/after=0` with client-side starvation lines still appearing rules out the host entirely (network/Quest-side). Also check `powerprofilesctl get` mid-session (should be `performance` while `vrserver` runs, held by the `steamvr-power-profile` service — confirm with `powerprofilesctl list-holds`) and try `pw-metadata -n settings 0 clock.force-rate 48000` to rule out a graph rate switch reconfiguring the binaural filter-chain mid-session | [0016](adr/0016-performance-power-profile-during-vr.md) |
