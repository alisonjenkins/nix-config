...
- **Launch surfaces:** Gaming Mode = add wrapped `retrofe` as a **single** non-Steam shortcut (via Steam ROM Manager); Plasma = `.desktop` entry. Validate gamescope fullscreen + controller focus (open question).
- **Steam Input (key simplification):** because RetroFE is the *only* Steam shortcut and the emulators are its **child processes**, the whole stack uses **one Steam Input layout** (RetroFE's). In Gaming Mode, Steam Input hands RetroFE + children one virtual xbox360 pad, so the emulators' native configs (03) map a single consistent device. No per-emulator `.vdf`/per-AppID binding. See 03 §3.3.
- **Per-game emulator routing:** a `platforms.<name>.games` entry of `{ file; emulator; }` selects which installed backend launches that game. RetroFE supports per-item launcher overrides; the module generates the per-collection launcher + per-game override from this (mechanism TBD at build — confirm RetroFE's exact per-item launcher format).
